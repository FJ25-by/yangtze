    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('kicauan -_-') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ berlian Anda habis\ngunakan perintah berikut untuk membeli lebih banyak berlian\n*${usedPrefix}buy* <jumlah> \n*${usedPrefix}buyall*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `✳️ level yang dibutuhkan ${plugin.level} untuk menggunakan perintah ini. \nLevel Anda ${_user.level}`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`Anda menggunakan *${+m.diamond}* 💎`)
                }
                break
            }
        }
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Desconocido') :
                            ...
                            let wel = API('https://api.fgmods.xyz', '/api/welcome', {
                            ...
                            let lea = API('https://api.fgmods.xyz', '/api/goodbye', {
                                username: await this.getName(user),
                                groupname: await this.getName(id),
                                groupicon: ppgp,
                                membercount: groupMetadata.participants.length,
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user sekarang dia adalah administrator')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.imgur.com/whjlJSf.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user bukan lagi pengurus')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Tautan grup berubah menjadi\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        mods: '🔰  Fungsi ini hanya untuk *FJ*',
        premium: '💠 Perintah ini hanya untuk *anggota Premium*\n\nKetik *.owner* untuk beli premium\n\nen: This command only for premium member\nsend the message ".owner" for buy premium',
        group: '⚙️ Perintah ini hanya dapat digunakan dalam grup!',
        private: '📮 Perintah ini hanya dapat digunakan di chat *pribadi dengan Bot*',
        admin: '🛡️ Perintah ini hanya untuk *Admin Grup*',
        botAdmin: '💥 Untuk menggunakan perintah ini saya harus menjadi *Administrator!*',
        unreg: '📇 Masuk untuk menggunakan fitur Pengetikan ini:\n\n*/reg nama.umur*\n\n📌 Example : */reg FJ.18*',
...
title: 'YANGTZE',
body: 'AKSES DITOLAK',
thumbnailUrl: 'https://telegra.ph/file/485ace354ab59724fb0ca.jpg',
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.magenta("✅  Itu telah diperbarui 'handler.js'"))
