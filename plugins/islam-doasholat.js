...
let handler = async (m, { args, usedPrefix, command }) => {
    let json = JSON.parse(JSON.stringify(global.asmaulhusn))
    let data = json.map((v, i) => `${i + 1}. ${v.latin}\n${v.arabic}\n${v.translation_id}`).join('\n\n')
    if (isNaN(args[0])) throw `contoh:\n${usedPrefix + command} 1`
    if (args[0]) {
        if (args[0] < 1 || args[0] > 99) throw `minimal 1 & maksimal 8!`
        let { id, name, latin, arabic, translation_id, terjemahan } = json.find(v => v.id == args[0].replace(/[^0-9]/g, ''))
        return m.reply(`No. ${id}
${name}
${arabic}
${latin}
${terjemahan}
...
handler.help = ['bacaan_shalat 1-8']
handler.tags = ['quran','islam']
handler.command = /^(bacaan_shalat)$/i
