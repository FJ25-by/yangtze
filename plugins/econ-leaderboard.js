...
▢ *TOP ${len} XP* 🧬
Kamu : *${usersExp.indexOf(m.sender) + 1}* daru *${usersExp.length}*
...
${sortedLim.slice(0, len).map(({ jid, diamond }, i) => `*${i + 1}.* ${participants.some(p => areJidsSameUser(jid, p.id)) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} ➭ _*Saldo ${diamond}*_`).join`\n`}
