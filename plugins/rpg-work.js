...
let handler = async (m, { conn, usedPrefix }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let str = `
${('healt')} Nyawa mu berkurang -${healt * 1} karena Kamu telah berpetualang sampai ${pickRandom(['🇯🇵 Jepang', '🇰🇷 Korea', '🇮🇳 India', '🇺🇲 Amerika', '🇵🇸 Palestin', '🇮🇶 Iraq', '🇸🇦 Arab', '🇵🇰 Pakistan', '🇩🇪 German', '🇫🇮 Finlandia', 'Ke bawa dunia mimpi 😱', 'Ujung dunia 🌏', 'Mars 👽', 'Bulan 🌚', 'Pluto 😱', 'Matahari 🌞', 'Hatinya dia ♥️', '...'])} dan mendapatkan
${('exp')} *exp:* ${exp} 
${('money')} *uang:* ${uang}
${('sampah')} *sampah:* ${sampah}${potion == 0 ? '' : `\n*${('potion')}Potion:* ` + potion + ''}${iron == 0 ? '' : `\n*${('iron')}Iron:* ` + iron + ''}${kayu == 0 ? '' : `\n*${('kayu')}Kayu:* ` + kayu + ''}${batu == 0 ? '' : `\n*${('batu')}Batu:* ` + batu + ''}${string == 0 ? '' : `\n*${('string')}String:* ` + string + ''}${diamond == 0 ? '' : `\n*${('diamond')}diamond:* ` + diamond + ''}${common == 0 ? '' : `\n*${('common')}common crate:* ` + common + ''}${uncommon == 0 ? '' : `\n*${('uncommon')}uncommon crate:* ` + uncommon + ''}`
`.trim()
            conn.reply(m.chat, str, m)
            if (mythic > 0) {
                   global.db.data.users[m.sender].mythic += mythic * 1
                   conn.reply(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + `${('mythic')}Mythic Crate`, m)
            }
            if (legendary > 0) {
                global.db.data.users[m.sender].legendary += legendary * 1
                conn.reply(m.chat, '*Selamat anda mendapatkan item Epic yaitu*\n' + legendary + `${('legendary')}Legendary Crate`, m)
            }
