...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `_*Saya Milik FJ Untuk Selamanya*_`, m)
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan] == m.sender && global.db.data.users[m.sender].pasangan != user){
      conn.reply(m.chat, `Kamu sudah berpacaran dengan @${global.db.data.users[m.sender].pasangan('@')[0]}\n\nSilahkan putus dulu (ketik .putus untuk memutuskan hubungan) untuk menembak @${user.split('@')[0]}\n\nBtw yang setia dikit banget!`, m , { contextInfo: { mentionedJid: [user, global.db.data.users[m.sender].pasangan]}})
...
"Walau aku tidak bisa memilikimu, aku akan tetap setia menunggumu.",
