...
*${usedPrefix + command}*  [item] [jumlah] [@user]
...
handler.before = async m => {
    ...
    if (/si?/g.test(m.text.toLowerCase())) {
        ...
        user[type] -= count * 1
        _user[type] += count * 1
        if (previous > user[type] * 1 && _previous < _user[type] * 1) m.reply(`✅ Pemindahan \n\n*${count}* *${type}*  ke @${(to || '').replace(/@s\.whatsapp\.net/g, '')}`, null, { mentions: [to] })
        else {
            user[type] = previous
            _user[type] = _previous
            m.reply(`❎ Pengalihan kesalahan *${count}* ${type} ke *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, { mentions: [to] })
        }
        clearTimeout(timeout)
        delete confirmation[sender]
    }
...
handler.tags = ['econ']
handler.command = ['payxp','paydi', 'transfer', 'darxp','dardi','tf']
