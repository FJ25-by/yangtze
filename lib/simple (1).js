...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { showAdAttribution: true, title: `${namebot}`, body: wm, sourceUrl: `${sig}`, thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
